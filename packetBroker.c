...
print_stats(void)
{
	...
	for (portid = 0; portid < 2; portid++)
	{
		printf("\nStatistics for port %u ------------------------------"
			   ...
			   "\nTCP RST to Client match: %22" PRIu64
			   "\nTCP RST to Server match: %14" PRIu64
...
{
	fprintf(f, "ps_id,rstClient,rstServer,no_match,rx_0_count,tx_0_count,rx_0_size,tx_0_size,rx_0_drop,rx_0_error,tx_0_error,rx_0_mbuf,rx_1_count,tx_1_count,rx_1_size,tx_1_size,rx_1_drop,rx_1_error,tx_1_error,rx_1_mbuf,time,throughput\n"); // Header row
}
...
populate_json_array(json_t *jsonArray, char *timestamp){
	...
	json_object_set(jsonObject, "rstClient", json_integer(port_statistics[0].rstClient));
	json_object_set(jsonObject, "rstServer", json_integer(port_statistics[0].rstServer));
